...
async def login(request: LoginRequest, response: Response):
    ...
    try:
        ...
        FRONTEND_DOMAIN = "training-next-chi.vercel.app"
        # 設定HttpOnly Cookies
        response.set_cookie("access_token", supabase_response.session.access_token, httponly=True, secure=True, domain=FRONTEND_DOMAIN, max_age=60*60)
        response.set_cookie("refresh_token", supabase_response.session.refresh_token, httponly=True, secure=True, domain=FRONTEND_DOMAIN, max_age=60*60*24*30)
